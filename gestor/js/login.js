...
document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .forEach((el) =>
      el.addEventListener("change", () =>
        Preferences.set({ [el.dataset.pref]: el.value }),
      ),
    );
  ...
  const setMode = (mode) => {
    ...
    registerForm.hidden = !create;
    document.querySelector("#mode-login").classList.toggle("primary", !create);
    document
      .querySelector("#mode-register")
      .classList.toggle("primary", create);
    note.textContent = create
      ? "Crie uma conta local para separar seus clientes e cobranças neste navegador."
      : "Entre com o usuário e a senha criados neste navegador.";
    (create ? registerForm : loginForm).querySelector("input")?.focus();
  };
  document
    .querySelector("#mode-login")
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      await Auth.login(loginForm.username.value, loginForm.password.value);
      location.href = "./painel.html?v=37#dashboard";
    } catch (err) {
  ...
  if (users.length)
    document.querySelector("#known-users").textContent =
      "Usuários neste navegador: " + users.join(", ");
  setMode("login");
  const params = new URLSearchParams(location.search);
  if (params.has("logout"))
    note.textContent =
      "Sessão encerrada. Entre novamente para acessar o painel.";
